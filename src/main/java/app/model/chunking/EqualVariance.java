...
public class EqualVariance extends ChunkingStrategy
{
    ...
    public EqualVariance(int base_number_of_chunks, int base_doc_id, String base_doc_name, Logger chunkingStrategyLogger)
    {
...
//                    base_doc_name = document.getFileName();
        ...
        if (baseNumberOfWords < base_number_of_chunks)
            throw new TaskException("Base document must have at least the same number of words as the base number of chunks");
        ...
        for (Document document : documents)
        {
            ...
            {
                numChunks = (int) calculateNumChunks(base_number_of_chunks, baseNumberOfWords, document.getNumWords(), document.getFileName());
            }
            docIdToVariance.put(document.getId(), calculateVariance(numChunks, document.getNumWords()));
            chunkingStrategyLogger.log(LogLevel.TRACE, String.format("doc_%d split in %d chunks, variance: %f", document.getId(), numChunks, docIdToVariance.get(document.getId())));
    ...
    public double calculateVariance(int numChunks, int totalWords)
    {
        ...
        if (totalWords <= 1)
        {
//            chunkingStrategyLogger.log(LogLevel.WARNING, String.format("total number of words for document %s is <=1, only created 1 chunk for it", documentName));
//            warningHandler.handleWarning(String.format("total number of words for document %s is <=1, only created 1 chunk for it", documentName));
            return 0;
        }
    ...
    public long calculateNumChunks(int baseNumberChunks, int baseTotalWords, int currentDocumentTotalWords, String documentName)
    {
        long result;
        ...
        {
            double resultDouble = (double) 1 + ((double) (currentDocumentTotalWords - 1) * calculateVariance(baseNumberChunks, baseTotalWords));
    ...
    public static class EqualVarianceEvaluationDataPage extends DocumentDataPage
    {
        ...
        public EqualVarianceEvaluationDataPage(Map<Integer, Double> docIdToVariance, Map<Integer, Integer> docIdToNumChunks, Map<Integer, Integer> docIdToChunkSize)
        {
        ...
        public List<Cell> generateExtraDocumentDetails(Document document)
        {
            ...
            return List.of(
                    (docIdToVariance.containsKey(document.getId()) ?
                            new Cell(docIdToVariance.get(document.getId())) :
