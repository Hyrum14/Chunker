...
public abstract class ValidateDocumentTask extends Task<Document>
{
    ...
    public ValidateDocumentTask(TaskHandler<Document> observer, Logger logger, Document document, boolean forceUpdate, DocumentDao documentDao)
    {
    ...
    public boolean validate()
    {
        try
        {
            ...
            if (documentIdOption.isPresent())
            {
                ...
                {
                    handler.handleFailure(document, String.format(DOCUMENT_ALREADY_EXISTS_IN_DATABASE, document.getFileName()));
                    return false;
                }
                isForceUpdate = true;
            } else
